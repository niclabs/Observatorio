...
func GetNSs(run_id int, dbname string, user string, password string){
	mutexTT = &sync.Mutex{}
	t:=time.Now()
	c:=30
...
func saveNSs(run_id int, db *sql.DB){
	rows, err:=dbController.getNSs(run_id, db)
	if(err!=nil){
		panic(err)
	}
	defer rows.Close()
	filename:="csvs/"+strconv.Itoa(run_id)+"domainNSs.csv"
	file, err := os.Create(filename)
	if(err!=nil){
		panic(err)
	}
